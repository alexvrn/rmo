// Local
#include "GraphicShP.h"

GraphicShP::GraphicShP(QWidget *parent)
  : AbstractGraphic(parent)
  , m_thread(new QThread(this))
  , m_worker(new GraphicShPWorker())
{
  m_thread->start();
  m_worker->moveToThread(m_thread);

  connect(m_worker, &GraphicShPWorker::calculatedData, this, &GraphicShP::calculatedData, Qt::QueuedConnection);
...
void GraphicShP::calculateData(const QList<QVariantMap>& data, bool isNowData, int seconds, int shiftData,
...
void GraphicShP::calculatedData(const QHash<QPair<int, int>, double>& result, int keySize, int valueSize, int yRange,
...
void GraphicShP::setGradient(const QCPColorGradient& gradient)
{
  m_colorMap->setGradient(gradient);
}


void GraphicShP::mouseMove(QMouseEvent* event)
{

}


void GraphicShP::mousePress(QMouseEvent* event)
{
